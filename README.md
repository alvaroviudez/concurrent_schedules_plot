...
**Back-to-back bar plot** — one row per reinforcer delivered during the session, on either schedule, plus a final row for any responses made after
the last reinforcer if the session didn't end on a reinforced trial. Each bar is the number of responses emitted on that schedule *since that schedule last produced a reinforcer*, with A extending left and B extending right. Full color marks the schedule that produced the reinforcer at that point; the lighter shade means that schedule's count is still running:
...
The **back-to-back bar plot** trades the time axis away to get exactly that back. Every bar starts from a shared zero baseline, so response counts are read against a common scale, and the A-vs-B comparison within a row is immediate rather than inferred. Reinforcement status is encoded on the bar itself through color saturation, so responding and its consequence are co-located instead of split between a line and a separate tick mark. The unit is a reinforcer delivery, not a fixed slice of time, which makes the display independent of whether the schedules in use are ratio-based, interval-based, or anything else. The cost is that time becomes ordinal: rows are evenly spaced no matter how much time elapsed between one reinforcer and the next, so response *rate* cannot be recovered from this plot, and within-run structure such as pausing is invisible. One further cost: when a schedule goes several trials without being reinforced, its bar repeats the same running count — still light-shaded — in each of those rows. Those rows are the same accumulated total shown multiple times, not independent magnitudes, so comparing bar lengths across a stretch of unreinforced trials on one side does not carry the same information as comparing the two sides of a single row.
