...
**Back-to-back bar plot** — one row per reinforcer delivered during the session, on either schedule. Each bar is the number of responses emitted on that schedule *since that schedule last produced a reinforcer*, with A extending left and B extending right. Full color marks the schedule that produced the reinforcer at that point; the lighter shade means that schedule's count is still running:
